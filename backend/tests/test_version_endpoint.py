...
def test_version_ok() -> None:
    ...
    assert data["sheet_webhook_host"] is None
    caps = data["sheet_webhook_capabilities"]
    assert caps["follow_redirects"] is True
    assert caps["reject_success_only_json"] is True
