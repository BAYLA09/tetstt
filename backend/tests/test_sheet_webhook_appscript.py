"""Google Apps Script returns HTTP 200 + JSON { ok: false } on some errors."""
...
def test_send_order_to_sheet_treats_appscript_ok_false_as_failed() -> None:
    async def _run() -> None:
        mock_resp = MagicMock()
        mock_resp.raise_for_status = MagicMock()
        mock_resp.json = MagicMock(return_value={"ok": False, "error": "Unauthorized"})
