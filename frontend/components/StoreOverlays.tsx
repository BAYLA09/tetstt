...
function CartDrawer() {
  ...
  return <AnimatePresence>{isCartOpen && <>
    ...
    <motion.aside className="drawer" initial={{ x: 80, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 80, opacity: 0 }} aria-label="سلة الطلب">
      ...
      {!!crossSells.length && <section><h3>أضيفي لمسة تكمل طلبك</h3>{crossSells.map((product) => <div key={product.sku} className="card" style={{ padding: 14, margin: "10px 0" }} data-testid={`cross-sell-${product.sku}`}><span className="badge">{product.sku === "LB-SERUM-SET-99" ? "الأكثر إضافة مع الباقة" : "ترقية ذكية"}</span><h4>{product.name}</h4><p>{product.subheading}</p><button className="btn secondary" type="button" aria-label={`أضيفي ${product.name} بسعر AED ${product.price}`} onClick={() => addProduct(product)}>أضيفيه AED {product.price}</button></div>)}</section>}
